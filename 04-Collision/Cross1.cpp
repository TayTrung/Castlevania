...
void Cross1::Update(DWORD dt, vector<LPGAMEOBJECT>* colliable_objects)
{
	// Calculate dx, dy 
...
void Cross1::SetState(int state)
{
	this->state = state;
}

void Cross1::GetBoundingBox(float & left, float & top, float & right, float & bottom)
{
	left = x;
	top = y;
	right = x + CROSS1_BBOX_WIDTH;
	bottom = y + CROSS1_BBOX_HEIGHT;
}
